...
public class FlowerShopOrderTests
{
	...
	{
		// arrange
		FlowerShopOrder order = new FlowerShopOrder("one rose", 1);
		...
		// assert
		Assert.assertEquals("because bouquets with 1 rose have a standard price of 22.98", expected, actual);
	}
	...
	{
		// arrange
		FlowerShopOrder order = new FlowerShopOrder("massive appology", 100);
		...
		// assert
		Assert.assertEquals("because bouquets with 100 roses cost 299", expected, actual);
	}
	...
	public void calculateDelivery_waivesFee_for10000_19999ZipCodes()
	{
		...
		// assert
		Assert.assertEquals("close delieveries are free", expected, actual);
	}
	...
	@Test
	public void calculateDelivery_charges1999_forInvalidZipCodes()
	{
		// arrange
		FlowerShopOrder order = new FlowerShopOrder("standard", 0);
		boolean isSameDay = false;
		...
		// assert
		Assert.assertEquals("because invalid zip codes should default to the max price of 19.99", expected, actual);
	}
