...
public class Exercises {
	...
	{
		boolean arrayLengthAtLeastOne = nums.length > 0;
		boolean doesFirstEqualLast = nums[0] == nums[nums.length -1];
		
		return arrayLengthAtLeastOne && doesFirstEqualLast;
	}
	...
	public boolean commonEnd(int[] a, int[] b)
	{
		...
		boolean isLastElementSameElementSame = a[a.length - 1] == b[b.length - 1];
		
		boolean hereIsTheResult = isFirstElementSame || isLastElementSameElementSame;
	...
	/*
	 10. Given 2 int arrays, a and b, each length 3,
	...
	/*
	 13. Given an array of ints, return true if the array contains a 2 next to a
	 2 somewhere.
	 has22([1, 2, 2]) → true
	 has22([1, 2, 1, 2]) → false
	 has22([2, 1, 2]) → false
	...
	/*
	 15. Given an array of ints, return true if the sum of all the 2's in the array
	 is exactly 8.
